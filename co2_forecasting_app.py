...
model = pickle.load(
    open(
        "/Users/geet/Documents/Projects_Git/EcoTrendPredictor/CO2_forecast_Model.pkl",
        "rb",
    )
)
...
year = st.slider(
    "Select number of Years", 1, 30, step=1
)  # creates a slider widget where the user can select a
# number of years between 1 and 30.


pred = model.forecast(
    year
)  # predict future values based on the selected number of years.
